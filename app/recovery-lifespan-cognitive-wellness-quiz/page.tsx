...
import {
  ...
  calculatePillar4Scores,
  getTopPillar4Classes,
  isFlatPillar4Profile,
} from "@/lib/pillar4-quiz";
import {
  getPillar4ClassBySlug,
  Pillar4ClassSlug,
  Pillar4ClassData,
} from "@/lib/pillar4-classes";
import { ArrowLeft } from "lucide-react";
import BookingModal from "@/components/BookingModal";

const ADVANCE_DELAY_MS = 280;

export default function RecoveryLifespanCognitiveWellnessQuizPage() {
  const [current, setCurrent] = useState(0);
  const [answers, setAnswers] = useState<number[]>(() =>
    Array(pillar4QuizQuestions.length).fill(-1)
  );
  const [showResults, setShowResults] = useState(false);
  const [resultClass, setResultClass] = useState<Pillar4ClassData | null>(null);
  const [affinities, setAffinities] = useState<Pillar4ClassSlug[]>([]);
  ...
  const showResultsFromAnswers = (finalAnswers: number[]) => {
    const scores = calculatePillar4Scores(finalAnswers);
    const { primary, affinities: aff } = getTopPillar4Classes(scores);
    const primaryData = getPillar4ClassBySlug(primary);

    if (primaryData) {
      setResultClass(primaryData);
      setAffinities(aff);
      setFlatProfile(isFlatPillar4Profile(finalAnswers, scores));
      setShowResults(true);
    }
  };
  ...
  const selectAnswer = (optionIndex: number) => {
    ...
    window.setTimeout(() => {
      ...
      if (questionIndex < pillar4QuizQuestions.length - 1) {
  ...
  const restart = () => {
    advancingRef.current = false;
    const fresh = Array(pillar4QuizQuestions.length).fill(-1);
  ...
  const currentQuestion = pillar4QuizQuestions[current];
  const progress = ((current + 1) / pillar4QuizQuestions.length) * 100;
      ...
      {!showResults && currentQuestion ? (
        <>
          <div className="mb-8">
            <div className="flex justify-between text-sm mb-2 text-brand-dark/60">
              <div>
                Question {current + 1} of {pillar4QuizQuestions.length}
              </div>
        ...
        <div className="text-center">
          ...
          <div className="inline-block mb-4 px-3 py-1 rounded-full bg-brand-blue/10 text-brand-blue text-xs font-medium">
            PILLAR 4 · RECOVERY &amp; LIFESPAN COGNITIVE WELLNESS
          </div>
          ...
          {flatProfile && (
            <p className="max-w-xl mx-auto text-lg text-brand-dark/80 mb-6 px-4 py-4 rounded-xl bg-brand-blue/5 border border-brand-blue/10">
              Your responses suggest you may be carrying brain load across several recovery and
              wellness patterns right now — and that&apos;s more common than you might think. Many
              people in seasons of transition, stress, or cognitive change still benefit from
              understanding their brain patterns more deeply, or booking a QEEG assessment for a
              clearer picture.
            </p>
          )}
          ...
          {affinities.length > 0 && (
            <div className="mb-10">
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                {affinities.map((slug) => {
                  const aff = getPillar4ClassBySlug(slug);
                  return aff ? (
